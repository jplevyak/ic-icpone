...
let size = await actor.stable_size();
console.log('stable_size', size);
let BLOCK_SIZE = 1024 * 1024;
let blocks = [];
let offset = 0;
size = Number(size);
fs.writeFileSync("stable_memory.dat", '');
while (size > 0) {
  let s = size;
  if (s > BLOCK_SIZE) {
    s = BLOCK_SIZE
  }
  console.log('stable_read', offset, s);
  let b = await actor.stable_read(offset, s);
  fs.appendFileSync("stable_memory.dat", b);
  size -= s;
  offset += s;
}
